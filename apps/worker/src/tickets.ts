...
function ticketSelect(): string {
  return `
    SELECT id, token, guest_name, ticket_type, created_at, voided_at
    FROM tickets
  `;
}

async function createTicket(request: Request, env: Env): Promise<Response> {
  let body: { guestName?: unknown; ticketType?: unknown };
  try {
  ...
  for (let attempt = 0; attempt < TOKEN_INSERT_ATTEMPTS; attempt += 1) {
    ...
    const created = await env.DB.prepare(`
      INSERT OR IGNORE INTO tickets (token, guest_name, ticket_type)
      VALUES (?, ?, ?)
      RETURNING id, token, guest_name, ticket_type, created_at, voided_at
    `).bind(token, guestName, ticketType).first<TicketRow>();
...
async function listTickets(env: Env): Promise<Response> {
  const result = await env.DB.prepare(`${ticketSelect()} ORDER BY created_at DESC, id DESC LIMIT 500`)
    .all<TicketRow>();
  return json({ tickets: result.results.map(ticketFromRow) });
}
